...
int ms_burst(struct Client* cptr, struct Client* sptr, int parc, char* parv[])
{
	...
	for (member = chptr->members; member; member = member->next_member)
		member->status &= ~(CHFL_BURST_JOINED | CHFL_BURST_ALREADY_OPPED | CHFL_BURST_ALREADY_VOICED
                        | CHFL_BURST_ALREADY_HALFOP | CHFL_BURST_ALREADY_ADMIN
                        | CHFL_BURST_ALREADY_OWNER | CHFL_BURST_ALREADY_SERVICE);
		...
		default: /* parameter contains clients */
		{
			...
			for (nick = ircd_strtok(&p, nicklist, ","); nick;
					...
					if (parse_flags & MODE_PARSE_SET) {
						...
						for (current_mode_needs_reset = 1; *ptr; ptr++) {
							...
							// Owner (+q)
								...
								current_mode |= CHFL_OWNER | CHFL_CHANNEL_MANAGER;
							}
							// Admin (+a)
							else if (*ptr == 'a') {
								if (current_mode_needs_reset) {
				...
				if (!(member = find_member_link(chptr, acptr)))
				{
					...
					// MODE-Kommandos nur für neu gesetzte Modi
					struct ModeBuf mbuf;
					int mode_sent = 0;
					struct Membership* existing_member = find_member_link(chptr, acptr);
					...
					if ((join_flags & CHFL_CHANSERVICE) && !(existing_member && IsChanService(existing_member)))
					{
					...
					if ((join_flags & CHFL_ADMIN) && !(existing_member && IsAdmin(existing_member)))
					{
						modebuf_mode_client(&mbuf, MODE_ADD | MODE_ADMIN, acptr, 0);
						mode_sent = 1;
					}
					if ((join_flags & CHFL_CHANOP) && !(existing_member && IsChanOp(existing_member)))
					{
						modebuf_mode_client(&mbuf, MODE_ADD | MODE_CHANOP, acptr, oplevel);
						mode_sent = 1;
					}
					if ((join_flags & CHFL_HALFOP) && !(existing_member && IsHalfOp(existing_member)))
					{
						modebuf_mode_client(&mbuf, MODE_ADD | MODE_HALFOP, acptr, 0);
						mode_sent = 1;
					}
					if ((join_flags & CHFL_VOICE) && !(existing_member && HasVoice(existing_member)))
					{
						modebuf_mode_client(&mbuf, MODE_ADD | MODE_VOICE, acptr, 0);
						mode_sent = 1;
					}
				...
				{
					// Member war schon im Channel, setze BURST_ALREADY_* Flags
					...
					if (member->status & CHFL_HALFOP)
						member->status |= CHFL_BURST_ALREADY_HALFOP;
	...
	if (parse_flags & MODE_PARSE_SET) { /* any modes changed? */
		...
		for (member = chptr->members; member; member = member->next_member) {
			if (member->status & CHFL_BURST_JOINED) { /* joined during burst */
				if ((member->status & CHFL_HALFOP) && !(member->status & CHFL_BURST_ALREADY_HALFOP)) {
					modebuf_mode_client(mbuf, MODE_ADD | MODE_HALFOP, member->user, OpLevel(member));
				}
				if ((member->status & CHFL_ADMIN) && !(member->status & CHFL_BURST_ALREADY_ADMIN)) {
					modebuf_mode_client(mbuf, MODE_ADD | MODE_ADMIN, member->user, OpLevel(member));
				}
				if ((member->status & CHFL_OWNER) && !(member->status & CHFL_BURST_ALREADY_OWNER)) {
					modebuf_mode_client(mbuf, MODE_ADD | MODE_OWNER, member->user, OpLevel(member));
				}
				if ((member->status & CHFL_CHANSERVICE) && !(member->status & CHFL_BURST_ALREADY_SERVICE)) {
					modebuf_mode_client(mbuf, MODE_ADD | MODE_CHANSERVICE, member->user, OpLevel(member));
				}
